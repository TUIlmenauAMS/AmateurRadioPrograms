...
Models have a context and output length limit. This means currently program code blocks to be made at once should have a maximum size that corresponds roughly to it, maybe a few thousand lines of code text or a few hundred kb at once (although that is changing quickly, check your models context and output limit). This means, larger projects need to be subdivided.
